...
for epoch in range(max_epoch):
    ...
    # Training Break
    if len(epoch_loss) > 1:
        if (np.abs(epoch_loss[-1] - epoch_loss[-2])/epoch_loss[-2])*100 <= 0.05:
            break
    # Save model
    model.save_pretrained("./models/roberta-quail-baseline/roberta-batched-seq-classify-epoch-"+str(epoch)+'.model')
